...
```{r message = FALSE, warning = FALSE, echo = FALSE}
  ...
  group_by(Country_Region)%>%
  summarise(Confirmed = sum(Confirmed), Deaths = sum(Deaths), Long = median(Long), Lat = median(Lat))
```

```{r message = FALSE, warning = FALSE, echo = FALSE}
  ggplot(daily_report_9_26_20, aes(x = Long, y = Lat, text = Country_Region, size = Deaths/1000)) +
    borders("world", colour = "black", fill = "grey90") +
    theme_bw() +
    geom_point(shape = 21, color='purple', fill='purple', alpha = 0.7) +
...
```{r message = FALSE, warning = FALSE,echo = FALSE}
   daily_report_9_26_20_US <- read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-26-2020.csv")) %>% 
  rename(Long = "Long_")%>%
  filter(Country_Region == "US") %>% 
  filter (!Province_State %in% c("Alaska","Hawaii", "American Samoa",
                  "Puerto Rico","Northern Mariana Islands", 
...
```{r message = FALSE, warning = FALSE,echo = FALSE}
  mybreaks <- c(1, 100, 1000, 10000, 10000)
  ggplot(daily_report_9_26_20_US, aes(x = Long, y = Lat, size = Confirmed)) +
    borders("state", colour = "white", fill = "grey90") +
    geom_point(aes(x=Long, y=Lat, size=Confirmed, color=Confirmed),stroke=F, alpha=0.7) +
    scale_size_continuous(name="Cases", trans="log", range=c(.5,2), 
                        breaks=mybreaks, labels = c("1-99",
                        "100-999", "1,000-9,999", "10,000-99,999", "50,000+")) +
    scale_color_viridis_c(option="viridis",name="Cases",
                        trans="log", breaks=mybreaks, labels = c("1-99",
...
ggplot(data = us, mapping = aes(x = long, y = lat, group = group)) + 
  ...
  borders("state", colour = "black") +
  geom_polygon(data = state_join, aes(fill = Confirmed)) +
  scale_fill_gradientn(colors = brewer.pal(n = 8, name = "Blues"),
                       breaks = c(1, 10, 100, 1000, 10000, 100000),
                       trans = "log10", na.value = "White") +
  ggtitle("Number of Confirmed Cases by US County") +
  theme_minimal() 
...
```{r message = FALSE, warning = FALSE,echo = FALSE}
  daily_report_9_26_20 <-   read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-26-2020.csv")) %>% 
    ...
    mutate(Admin2 = tolower(Admin2))
  us <- map_data("state")
  ma_us <- subset(us, region == "connecticut")
  counties <- map_data("county")
  ma_county <- subset(counties, region == "connecticut")
  state_join <- left_join(ma_county, daily_report_9_26_20, by = c("subregion" = "Admin2")) 
  # plot state map
  ggplot(data = ma_county, mapping = aes(x = long, y = lat, group = group)) + 
    coord_fixed(1.3) + 
  # Add data layer
    geom_polygon(data = state_join, aes(fill = Confirmed), color = "white") +
      scale_fill_gradientn(colors = brewer.pal(n = 5, name = "Blues"),
                           trans = "log10") +
    labs(title = "COVID-19 Confirmed Cases in Connecticut'")
```
...
Data from [@CSSEGISandData]
Referenced web-pages [@Bookdown] [@ggplot2] [@stackoverflow].
#Reference